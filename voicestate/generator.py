...
class generator():
    ...
    async def join(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        ...
        if str(after.channel.id) == data[str(member.guild.id)]['gen_id']:
            ...
            channel = await member.guild.create_voice_channel(name=f'{member.display_name}', category=category, reason='Voice Channel Generator')
    ...
    async def leave(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        data = self.client.jopen(f'Data/generator', str(member.guild.id))
