...
class catlink_(commands.Cog):
                    ...
                    for key, value in reactions.items():
                        if value == reaction.emoji:
                            try:
                                ...
                                jsonname = f"category\\cat{ctx.guild.id}"
    ...
    @commands.has_permissions(administrator=True)
    async def catunlink(self, ctx):
        complete = False
        category_list = []
        vcs = {}
        all_linked_bool = False

        categories = dis.jopen(f'category\\cat{ctx.guild.id}')

        if 'all' in categories:
            all_linked_bool = True
            all_linked = categories['all']
            categories.pop('all')

        try:        
            category_list, values = zip(*categories.items())
            ...
            catunlink_embed = discord.Embed(
                colour=discord.Colour.dark_grey(),
                description=f'All of the linked categories in {ctx.guild.name}\nPlease react with the category that you would like to unlink.'
            )
            if total_page > 1:
                catunlink_embed.set_footer(text=f'Page {cur_page}/{total_page}')
            ...
            for name_ in category_list:
                try:
                    name_ = self.client.get_channel(int(name_))
                    category_names.append(name_.name)
                    if cur_page * 9 >= category_num:
                        catunlink_embed.add_field(name=f'  {reaction_type[num]}', value=f'{name_.name}', inline=True)
                        category_num = category_num + 1
                except:
                    pass

                num = num + 1
            message = await ctx.send(embed=catunlink_embed)
            ...
            while True:
                try:
                    ...
                    if str(reaction.emoji) == "▶️" and cur_page != total_page:
                        ...
                        num = 1
                        cur_id = (cur_page-1)*9
                        while cur_id < len(category_list):
                            if cur_id < cur_page*9:
                                link_embed.add_field(name=f'  {reaction_type[num]}', value=f'{category_names[cur_id]}', inline=True)
                            num += 1
                            cur_id += 1                     
                        ...
                        for key, value in reactions.items():
                            if value == reaction.emoji:
                                try:
                                    id_pos = key - 1 
                                    categories = category_list[id_pos+(cur_page-1)*9]

                                    category = dis.jopen(f'category\\cat{ctx.guild.id}')

                                    category.pop(str(categories))

                                    dis.jdump(f'category\\cat{ctx.guild.id}', category)

                                    unlink_embed = discord.Embed(
                                        colour=discord.Colour.dark_grey(),
                                        description=f'Unlinked category {category_names[int(id_pos+(cur_page-1)*9)]} from it\'s role'
                                    )
            ...
            error_embed = discord.Embed(
                colour=discord.Colour.red(),
                description='**Error 122:** You do not have categories available to unlink'
            )
