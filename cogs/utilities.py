...
class utilities(commands.Cog):
    ...
    async def about(self, ctx):
        ...
        embed.add_field(name='__**Server Count**__',value=f'VC Roles is in {len(self.client.guilds)} servers', inline=False)
        embed.add_field(name='__**Current Bot Version**__', value='Bot Version - 2.0',inline=False)
        shard_info = self.client.get_shard(0)
        shard_id = shard_info.id 
        shard_count = shard_info.shard_count
        embed.add_field(name='__**Shard Info**__', value=f'Shard {shard_id}/{shard_count}')

        embed.add_field(name='__**Authors**__',value=f'**cde#4572** and **SamHartland#9376**',inline=False)
